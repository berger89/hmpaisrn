...
void fetchUpcomingLaunchesAction(Store<AppState> store) async {
  ...
  store.dispatch(
      new FetchSetUpcomingLaunchesAction(upcomingLaunch: fetchedLaunch));
}

ThunkAction<AppState> fetchPreviousLaunchesAction(
    DateTime startdate, DateTime enddate) {
  return (Store<AppState> store) async {
    store.dispatch(new FetchSetLoadingAction(loading: true));

    final fetchedLaunch =
        await fetchPreviousLaunches(startdate: startdate, enddate: enddate);

    store.dispatch(new FetchSetLoadingAction(loading: false));

    store.dispatch(
        new FetchSetPreviousLaunchAction(previousLaunch: fetchedLaunch));
  };
}
...
ThunkAction<AppState> fetchSearchLaunchesAction(int limit, String searchText) {
  return (Store<AppState> store) async {
    ...
    final fetchedLaunch =
        await fetchSearchLaunches(limit: limit, name: searchText);
